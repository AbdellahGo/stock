    ...
    return (
        <div className="mt-5 overflow-hidden rounded-xl border border-gray-200 shadow-sm md:overflow-hidden overflow-x-scroll">
            <table className="md:w-full text-left text-sm text-gray-600 ">
                <thead className="bg-[#f8f7f4] text-xs font-semibold uppercase tracking-wider text-[#a09d94] border-b border-gray-200">
                    <tr>
                ...
                {isPending || filteredProducts.length > 0 ?
                    <tbody className="divide-y divide-gray-200 bg-white font-medium">
                        {filteredProducts.map((product, i) => (
                            <tr key={product.id} className="hover:bg-[#f6f5f1] transition-colors">
                                <td className="px-6 py-2 text-[#a09d94]">{product.id}</td>
                                <td className="px-6 py-2 text-gray-900 font-bold">{product.name}</td>
                                <td className="px-6 py-2 text-center">
                                    <span className="inline-block px-3 py-1 rounded-full bg-[#e8e6df] text-[#6a6860] text-xs font-bold">
                                        {product.category_name}
                                    </span>
                                </td>
                                <td className="px-6 py-2 text-[#a09d94]">{product.supplier_name}</td>
                                <td className="px-6 py-2 text-gray-900 font-bold">{product.price} $</td>
                                <td className="px-6 py-2 text-center text-[#a09d94] font-normal">{product.quantity}</td>
                                <td className="px-6 py-2">
                                    <div className="flex justify-center items-center gap-2">
                                        <button onClick={() => setProductModal({
                                            ...
                                            className="p-2 rounded-lg border border-gray-100 hover:border-blue-300 hover:text-blue-400 hover:bg-blue-100 text-[#a09d94] transition-colors">
                                            <Eye size={18} />
                                        </button>
                                        <button onClick={() => setProductForm({
                                            ...
                                            className="p-2 rounded-lg border border-gray-100 hover:border-green-300 hover:text-green-400 hover:bg-green-100 text-[#a09d94] transition-colors">
                                            <Pencil size={18} />
                                        </button>
                                        <button onClick={() => setDeleteP({
                                            ...
                                            className="p-2 rounded-lg border border-gray-100 hover:border-red-300 hover:text-red-400 hover:bg-red-100 text-[#a09d94] transition-colors">
                                            <Trash2 size={18} />
                                        </button>
                                    </div>
                                </td>
            ...
            {productForm.isOpen ? <ProductForm product={products[productForm.productIndex]} action={productForm.action} setProductForm={setProductForm} /> : ''}
